    ...
    def semantic_parser(self, plan):
        ...
        for command in commands:
            ...
            if words[0] == "mov" or words[0] == "move":
                ...
                result.append("Mov " + self.get_noun_phrases(command, is_first_time))
            elif words[0] == "pick":
                self.log_info("Case VP2 NP1 NP3 (\"Pick Up\") found.")
                self.log_info("Identifying noun phrases...")
                result.append("Pick " + self.get_noun_phrases(command, is_first_time))
            elif words[0] == "put":
                self.log_info("Case VP3 NP1 NP4 (\"Put\") found.")
                self.log_info("Identifying noun phrases...")
                result.append("Put " + self.get_noun_phrases(command, is_first_time))
            else:
